...
**OAuth Migration:** The OAuth method in this endpoint is deprecated. Please use the new `/auth/social` endpoint instead:
...
### Swap Tokens ⚠️ Deprecated

- **Method:** `POST`
- **Path:** `/v1/payments/swap`
- **Tags:** Payments

**This endpoint is deprecated, use /v1/tokens/swap instead.** Swap one token for another using the optimal route available. You can specify a tokenIn amount (if exact='input') or tokenOut amount (if exact='output'), but not both. The corresponding output or input amount will be returned as the quote.
...
- **`data` (required)**
  ...
  **Items:**
  ...
  - **`developerFeeBps` (required)**

    `number`

  - **`developerFeeRecipient` (required)**
  ...
  - **`type` (required)**
    ...
    `string`, possible values: `"buy", "sell", "transfer"`
